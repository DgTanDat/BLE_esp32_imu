...
static void NotifyCallback(BLERemoteCharacteristic* pBLERemoteCharacteristic, 
  ...
  if(pBLERemoteCharacteristic->getUUID().toString() == medPayLoadCharacteristicUUID.toString()) {
    ...
    positionX += 0.5*(accX)*delta_t*delta_t + delta_t*curVecX;
    Serial.print("x position: ");
    Serial.println(positionX);
    positionY += 0.5*(accY)*delta_t*delta_t + delta_t*curVecY;
    Serial.print("y position: ");
    Serial.println(positionY);
    positionZ += 0.5*(accZ)*delta_t*delta_t + delta_t*curVecZ;
